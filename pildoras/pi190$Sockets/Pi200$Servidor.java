...
class MarcoServidor10 extends JFrame implements Runnable{ // Implementa Runnable para hacer 'Threads' para que est� permanentemente a la escucha de mensajes
  ...
  public void run() {
    ...
    try {
      ...
      while(true) { // Dentro de un bucle infinito para que acepte m�s mensajes
        ...
        paqueteRecibido = (PaqueteEnvio10) paqueteDatos.readObject(); // Lanza excepci�n de tipo 'ClassNotFoundException'
        ...
        if(!mensaje.equals("He conectado con el server")) { // Si ya ha conectado con el server antes
          ...
          for (String z : listaIp) { // Este bucle ForEach lo que hace es recorrer una a una las direcciones IP almacenadas en el 'ArrayList'
            ...
            // C�digo para crear un Socket (obtenido de dentro del if de este mismo else)
