...
class LaminaConImagen2 extends JPanel{
  ...
  public LaminaConImagen2() { // IOExeption = Excepci�n comprobada (lo programaste bien)
  ...
  public void paintComponent(Graphics g) {
    ...
    /* //Puede estar aqu� o dentro del constructor
    try { //Capturando la excepcion ya que es obligatorio
    ...
    if(imagen == null) { // Runtime Exception = Excepci�n no comprobada (lo podr�as haber programado mejor)
      g.drawString("No podemos cargar la imagen", 10, 30);
    } else {

      g.drawImage(imagen, 0, 0, null);

      int anchoImagen = imagen.getWidth(this); //This hace referencia a el objeto en que se encuentra, es decir la l�mina 'LaminaConImagen2'
      int altoImagen = imagen.getHeight(this);

      for(int i=0; i<90; i++) {
        for(int j=0; j<90; j++) {
          if(i+j>0){ //Porque la primera vez que pone la imagen la pondr� sobre la primera imagen que ya existe en la coordenada 0,0.
