...
class Banco {
	...
	public void transferencia(int cuentaOrigen, int cuentaDestino, double cantidad) { // Método para las transferencias
		
		if(cuentas[cuentaOrigen] < cantidad) { // Si el dinero de la cuenta de origen es menor que 'cantidad' (lo que se va a transferir)
			return;
		}
		
		System.out.println("El hilo que está haciendo la transferencia es: " + Thread.currentThread());
	...
	public double getSaldoTotal() { // Método para devolver el saldo total todas las cuentas a la vez
