package pi168$Threads; // Exlicaci�n del error de la programaci�n concurrente en los primeros 10 minutos del video 175
  ...
  public void transferencia(int cuentaOrigen, int cuentaDestino, double cantidad) { // M�todo que realiza las transferencias
    ...
    try {
      ...
      if(cuentas[cuentaOrigen] < cantidad) { // Si el dinero de la cuenta de origen es menor que 'cantidad' (lo que se va a transferir)
        return; // Esto convertir� el hilo en un hilo que solo habre y cierra pero no hace nada
      }
...
class EjecucionTransferencias2 implements Runnable{ // M�todo que har� transferencias infinitas
