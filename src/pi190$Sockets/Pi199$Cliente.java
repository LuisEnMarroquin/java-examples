...
public class Pi199$Cliente {

	public static void main(String[] args) {
		
		MarcoCliente9 mimarco=new MarcoCliente9();
...
class MarcoCliente9 extends JFrame{
	
	public MarcoCliente9(){
		
		setBounds(600,300,280,350);
				
		LaminaMarcoCliente9 milamina=new LaminaMarcoCliente9();
		
		add(milamina);
		
		setVisible(true);
		
		addWindowListener(new EstoyOnline3());
...
class EstoyOnline3 extends WindowAdapter{ // 'WindowAdapter' es una clase adaptadora de la interfaz 'WindowListener' para no tener que implementar todos los métodos de esta interfaz
	...
	public void windowOpened(WindowEvent e) { 
		...
		try {
			...
			PaqueteEnvio9 datos = new PaqueteEnvio9();
...
class LaminaMarcoCliente9 extends JPanel implements Runnable{ // Implementa 'Runnable' para que funcione como un Thread para recibir mensajes
	
	public LaminaMarcoCliente9(){
		...
		ip = new JComboBox();
		ip.addItem("192.168.1.102");
		ip.addItem("192.168.1.103");
		ip.addItem("192.168.1.104");
		add(ip);
	...
	private class EnviaTexto implements ActionListener{ // Creando clase interna para el botón de Enviar
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				PaqueteEnvio9 datos = new PaqueteEnvio9(); // Instanciando la clase 'PaqueteEnvio9'
				datos.setNick(nick.getText()); // Guardando lo que está dentro de las areas de texto en los Setters de la clase 'PaqueteEnvio9'
	...
	public void run() {
		try {
			...
			while(true) {
				...
				paqueteRecibido = (PaqueteEnvio9) flujoEntrada.readObject(); // Casting
...
class PaqueteEnvio9 implements Serializable{ // Clase que hará un paquete (objeto) con el nick de usuario, la dirección IP del destinatario y el mensaje // Implementa la interfaz 'Serializable' (que indica que la clase es susceptible a enviarse por la red) porque si no se implementa se lanzaria una excepción de tipo 'Serializable'
	...
	private ArrayList<String> Ips; // Creando 'ArrayList' donde se almacenarán las IP's de los usuarios que están conectados
