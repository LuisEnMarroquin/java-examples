...
public class Pi197$Cliente {

	public static void main(String[] args) {
		
		MarcoCliente7 mimarco=new MarcoCliente7();
...
class MarcoCliente7 extends JFrame{
	
	public MarcoCliente7(){
		
		setBounds(600,300,280,350);
				
		LaminaMarcoCliente7 milamina=new LaminaMarcoCliente7();
		
		add(milamina);
		
		setVisible(true);
		
		addWindowListener(new EstoyOnline());
...
class EstoyOnline extends WindowAdapter{ // 'WindowAdapter' es una clase adaptadora de la interfaz 'WindowListener' para no tener que implementar todos los métodos de esta interfaz
	
	public void windowOpened(WindowEvent e) { 
		
		try {
			
			Socket miSocket = new Socket("192.168.0.194", 9999); // Creando nuevo 'socket'
			
			PaqueteEnvio7 datos = new PaqueteEnvio7();
...
class LaminaMarcoCliente7 extends JPanel implements Runnable{ // Implementa 'Runnable' para que funcione como un Thread para recibir mensajes
	
	public LaminaMarcoCliente7(){
	...
	private class EnviaTexto implements ActionListener{ // Creando clase interna para el botón de Enviar
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				PaqueteEnvio7 datos = new PaqueteEnvio7(); // Instanciando la clase 'PaqueteEnvio7'
				datos.setNick(nick.getText()); // Guardando lo que está dentro de las areas de texto en los Setters de la clase 'PaqueteEnvio7'
	...
	public void run() {
		try {
			...
			while(true) {
				...
				paqueteRecibido = (PaqueteEnvio7) flujoEntrada.readObject(); // Casting
