...
public class Pi200$Servidor {
	
	public static void main(String[] args) {
		
		MarcoServidor10 mimarco=new MarcoServidor10();
...
class MarcoServidor10 extends JFrame implements Runnable{ // Implementa Runnable para hacer 'Threads' para que esté permanentemente a la escucha de mensajes
	
	public MarcoServidor10(){
	...
	public void run() {
		...
		try {
			...
			PaqueteEnvio10 paqueteRecibido; // Intancia de 'PaqueteEnvio10' llamada 'paqueteRecibido' // Hay que inicializar la variable para poder usarla
			...
			while(true) { // Dentro de un bucle infinito para que acepte más mensajes
				...
				paqueteRecibido = (PaqueteEnvio10) paqueteDatos.readObject(); // Lanza excepción de tipo 'ClassNotFoundException'
				...
				if(!mensaje.equals("He conectado con el server")) { // Si ya ha conectado con el server antes
					...
					for (String z : listaIp) { // Este bucle ForEach lo que hace es recorrer una a una las direcciones IP almacenadas en el 'ArrayList'
						
						// Código para crear un Socket (obtenido de dentro del if de este mismo else)
						Socket enviaDestinatario = new Socket(z, 9090); // Socket para enviar los datos al destinatario
						ObjectOutputStream paqueteReenvio = new ObjectOutputStream(enviaDestinatario.getOutputStream()); // Objeto de tipo 'ObjectOutputStream' llamado 'paqueteReenvio'
						paqueteReenvio.writeObject(paqueteRecibido);
						paqueteReenvio.close(); // Cerrando el flujo de datos
						enviaDestinatario.close(); // Cerrando el Socket de envio al destinatario
